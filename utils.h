...
//Code below turns on and off a LED connected to PORTB 18 (RED) and PORTB 19(GREEN)
//Assignment: Add blink of the onboard blue LED, red and green are already blinking
void blink(void){
    ...
    PTB->PDDR |= (3UL << 18);    /* make PTB as output pins */
		PTB->PDDR |= (3UL << 19);    /* make PTB as output pins */
		//ENABLE CLK TO BLUE LED PORT
		//MAKE BLUE LED PORT AS GPIO
		//MAKE BLUE LED PORT AS OUTPUT
		
    while(1){
				
        PTB->PSOR = (3UL << 18);         /* Set PB18, RED */
			  PTB->PSOR = (3UL << 19);         /* Set PB19, GREEN */
				//SET BLUE LED
        Delay(5000000);
        PTB->PCOR = (3UL << 18);				 /* Clear PTB18, RED*/
				PTB->PCOR = (3UL << 19);				 /* Clear PTB18, GREEN*/
				//CLEAR BLUE LED
			  Delay(5000000);
    }
...
//////////////////////////////////////OLD CODE//////////////////////////////////////////
if (PTD->PDIR & 0x20)	{
GPIOD_PDOR &= ~0x10;} /* turn off  LED */
else{
GPIOD_PDOR |= 0x10; }/* turn on  LED */
...
PORTD_PCR4 = 0x100; /* make PTB19 pin as GPIO (See Table 2-4) */
GPIOD_PDDR |= 0x10; /* make PTB19 as output pin */



PTD->PTOR = 0x10; /* toggle LED */
...
//	LCD_command(0x80);
//}
PORTD_PCR5 = 0x103; /* make PTD5 pin as GPIO and pull-up(See Table 2-4) */
GPIOD_PDDR &= ~0x20; /* make PTD5 as INPUT pin */	
